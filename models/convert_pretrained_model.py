...
def convert_resnet18(path=resnet_18_path):
    ...
    torch.save(model, './pretrained/resnet18_places365.pth.tar')
    return model
...
def convert_resnet50(path=resnet_50_path):
    ...
    torch.save(model, './pretrained/resnet50_places365.pth.tar')
    return model
...
def convert_se_resnet50(path=resnet_50_path):
    ...
    model.fc = nn.Linear(2048, 80)
    torch.save(model, './pretrained/se_resnet50_places365.pth')
    return model
